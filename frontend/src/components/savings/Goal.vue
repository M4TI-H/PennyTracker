...
<template>
  ...
  bg-[#E9ECEF] rounded-xl shadow-xl">
    ...
    <p class="text-xl font-bold text-neutral-800 mt-2">{{ goalTitle }}</p>
    <p class="text-3xl font-semibold text-neutral-800">${{ currentAmount }} / ${{ goalAmount }}</p>
    ...
    <span v-if="showDeposit" class="w-[100%] h-[10%] flex items-end justify-between mt-auto">
      <input type="number" placeholder="Deposit value" class="w-[70%] h-10 px-2 border-2 border-neutral-800 rounded-l-lg border-r-0 text-md font-medium focus:outline-0"/>
      <button @click="hideInput" class="w-[30%] h-10 rounded-r-lg bg-none font-semibold text-sm text-neutral-800 
